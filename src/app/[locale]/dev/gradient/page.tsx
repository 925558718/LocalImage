...
import { Copy, Plus, RotateCcw, Shuffle, Trash2 } from "lucide-react";
...
const GradientGenerator = () => {
	...
	const gradientBarRef = useRef<HTMLButtonElement>(null);
	...
	const generateTailwind = () => {
		...
		const getDirection = () => {
			if (config.type === "linear") {
				...
				const directions =
					tailwindVersion === "v4" ? v4Directions : v3Directions;
				return (
					directions[config.direction] ||
					(tailwindVersion === "v4" ? "bg-linear-to-r" : "bg-gradient-to-r")
				);
			}
		...
		if (sortedStops.length === 2) {
			...
			const fromColor = hexToTailwindColor(sortedStops[0].color);
			const toColor = hexToTailwindColor(
			...
			if (
				!fromColor.includes("[") &&
				!toColor.includes("[") &&
				middleStops.length <= 1
			) {
				const viaColor =
					middleStops.length > 0
						? hexToTailwindColor(middleStops[0].color)
						: "";
	...
	const handleDragStart = useCallback(
		(event: React.MouseEvent, stopId: string) => {
	...
	const handleDrag = useCallback(
		(event: MouseEvent) => {
			...
			const newPosition = Math.max(
				0,
				Math.min(100, dragging.startPosition + deltaPercentage),
			);
	...
	const simplifyGradient = () => {
		...
		if (sortedStops.length <= 3) {
			...
			toast.success(
				`Simplified to 3 colors: ${firstStop.color}, ${middleStop.color}, ${lastStop.color}`,
			);
		}
	};
			...
			<div className="min-h-screen w-full">
				<main className="container mx-auto px-4 py-12 max-w-7xl">
					...
					<div className="text-center mb-12">
						<h1 className="gradient-title mb-4">CSS Gradient Generator</h1>
						<p className="text-lg text-slate-600 dark:text-slate-400 max-w-2xl mx-auto leading-relaxed">
							Create beautiful gradients with live preview and copy-ready CSS or
							Tailwind code
						</p>
							...
							<CardContent>
								...
								<div className="space-y-2">
									<Label className="text-sm font-medium">
										Interactive Color Bar - Click to add, drag to move
									</Label>
									<button
										ref={gradientBarRef}
										...
										type="button"
										aria-label="Gradient color bar. Click to add color stop"
													...
													>
														...
														{config.stops.length > 2 && (
															<button
																...
																aria-label={`Delete color stop at position ${stop.position}%`}
															>
																<svg
																	width="12"
																	height="12"
									...
									<p className="text-xs text-muted-foreground">
										💡 Click anywhere to add a color stop, drag existing stops
										to reposition them, click a stop to edit it
									</p>
								</div>
							</CardContent>
						</Card>
							...
							<div className="space-y-4">
								<Card>
									...
									<CardContent className="space-y-4">
										...
										<div className="space-y-2">
											<Label>
												{config.type === "linear" ? "Direction" : "Shape"}
											</Label>
											<Select
								...
								<Card>
									...
									<CardContent>
										<Tabs
											value={outputFormat}
											onValueChange={(value: string) =>
												setOutputFormat(value as "css" | "tailwind")
											}
										>
											...
											<TabsContent value="tailwind" className="space-y-4">
												...
												<div className="space-y-2">
													...
													<Tabs
														...
														onValueChange={(value: string) =>
															setTailwindVersion(value as "v3" | "v4")
														}
													>
														<TabsList className="grid w-full grid-cols-2">
												...
												<div className="space-y-2">
													<Label>
														Tailwind CSS {tailwindVersion.toUpperCase()} Classes
													</Label>
													<div className="relative">
														<pre className="p-3 bg-muted rounded text-sm overflow-x-auto border">
												...
												{tailwindVersion === "v4" ? (
													<div className="p-3 bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded">
														<p className="text-sm text-blue-800 dark:text-blue-200">
															<strong>Tailwind CSS v4:</strong> Uses new
															gradient syntax (bg-linear-*, bg-radial-*). This
															is the latest version with improved gradient
															utilities.
														</p>
													</div>
												) : (
													<div className="p-3 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded">
														<p className="text-sm text-green-800 dark:text-green-200">
															<strong>Tailwind CSS v3:</strong> Uses traditional
															gradient syntax (bg-gradient-*). This is the
															stable version currently in wide use.
														</p>
													</div>
												)}
												...
												{tailwindGradient.includes("bg-[") && (
													<div className="p-3 bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded">
														<p className="text-sm text-yellow-800 dark:text-yellow-200">
															<strong>Complex Gradient:</strong> Using arbitrary
															value because:
															<br />• Colors not in Tailwind&apos;s standard
															palette, or
															<br />• More than 3 color stops (Tailwind supports
															max 3: from/via/to)
															<br />
															<br />
															<strong>Note:</strong> Tailwind CSS supports
															position syntax like:
															<br />• <code>from-blue-500 from-10%</code>
															<br />• <code>via-red-500 via-30%</code>
															<br />• <code>to-green-500 to-90%</code>
															<br />
															<br />
															<strong>Arbitrary Value Format:</strong>{" "}
															<code>bg-[linear-gradient(...)]</code>
														</p>
													</div>
												)}
											</TabsContent>
