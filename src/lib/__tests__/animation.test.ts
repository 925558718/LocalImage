...
import ffm_ins, { FFMPEG } from '../ffmpeg';
...
describe('FFmpeg 动画功能测试', () => {
	// 在测试环境中，ffm_ins 可能为 null，所以跳过这些测试
	const shouldSkip = !ffm_ins;
	
	beforeAll(async () => {
		if (shouldSkip) {
			console.log('跳过动画测试：FFmpeg实例在测试环境中不可用');
			return;
		}
	...
	describe('createAnimation 方法测试', () => {
		it.skipIf(shouldSkip)('应该正确处理多张图片的动画合成', async () => {
			const testFiles = [
		...
		it.skipIf(shouldSkip)('应该正确处理GIF格式输出', async () => {
			const testFiles = [
				createMockFile('frame1.png'),
				createMockFile('frame2.png')
			];
		...
		it.skipIf(shouldSkip)('应该在没有图片时抛出错误', async () => {
		...
		it.skipIf(shouldSkip)('应该在不支持的格式时抛出错误', async () => {
		...
		it.skipIf(shouldSkip)('应该使用正确的默认值', async () => {
		...
		it.skipIf(shouldSkip)('应该正确处理自定义参数', async () => {
			const testFiles = [createMockFile('frame1.png'), createMockFile('frame2.png')];
			const ffmpegInstance = (ffm_ins! as any).ffmpeg;
			const execSpy = vi.spyOn(ffmpegInstance, 'exec');
