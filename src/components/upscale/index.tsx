...
function UpscaleComposer() {
	...
	const handleDownload = useCallback(
		async (result: OutputType, index: number) => {
			if (result.url) {
				try {
						...
						<Card className="p-6">
							<div className="space-y-4">
								...
								<div className="text-sm text-muted-foreground">
									{t("total_processed")}: {upscaleResults.length} /{" "}
									{imageFiles.length}
								</div>
									...
									{upscaleResults.map((result, index) => (
										...
										>
											...
											<div className="text-sm text-muted-foreground space-y-1">
												<div>
													{t("upscaled_size")}: {result.width} × {result.height}
												</div>
												<div>
													{t("processing_time")}:{" "}
												...
												<div>
													{t("file_size")}: {Math.round(result.size / 1024)}KB
												</div>
											</div>
											{result.url && (
												<img
